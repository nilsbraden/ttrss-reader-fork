...
public class ApacheJSONConnector extends JSONConnector {
	...
	protected InputStream doRequest(Map<String, String> params) {
		...
		try {
			...
			if (client == null) client = HttpClientFactory.getInstance().getHttpClient(post.getParams());
			else client.setParams(post.getParams());

			// Add SSL-Stuff
			if (credProvider != null) client.setCredentialsProvider(credProvider);
		...
		try {
			...
			if (entity != null) instream = entity.getContent();
