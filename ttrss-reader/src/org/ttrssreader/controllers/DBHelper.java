...
public class DBHelper {
	...
	private static final String INSERT_FEEDS = "replace into " + TABLE_FEEDS + "(id, categoryId, title, url, unread) values (?, ?, ?, ?, ?)";
	private static final String INSERT_ARTICLES = "replace into " + TABLE_ARTICLES + "(id, feedId, title, isUnread, content, articleUrl, articleCommentUrl, updateDate) values (?, ?, ?, ?, ?, ?, ?, ?)";

	private static final String UPDATE_ARTICLES = "UPDATE " + TABLE_ARTICLES + " SET content=?, title=?, articleUrl=?, articleCommentUrl=?, updateDate=? WHERE id=? AND feedId=?"; 
...
//		deleteAll();
	...
	public void updateArticleContent(ArticleItem result) {
		...
		byte[] contentBytes = Base64.encodeBytesToBytes(result.getContent().getBytes());
	...
	public void purgeArticlesDays(Date olderThenThis) {
		db.execSQL(
				...
				" WHERE isUnread=0 AND updateDate<" + olderThenThis.getTime());
	}
	...
	public void purgeArticlesNumber(int number) {
		db.execSQL(
				"DELETE FROM " + TABLE_ARTICLES +
				" WHERE id in( " +
				" select id from " + TABLE_ARTICLES + " WHERE isUnread=0" +
	...
	public Map<String, List<ArticleItem>> getArticles(int maxArticles) {
		...
		Cursor c = db.query(TABLE_ARTICLES, null, null, null, null, null, "updateDate DESC", String.valueOf(maxArticles));
		if (c.isBeforeFirst()) {
			if (!c.moveToFirst()) {
				return ret;
			}
		...
		while (!c.isAfterLast()) {
			...
			ArticleItem fi = new ArticleItem(
					key,								// feedId
					c.getString(0),						// id
					c.getString(2),						// title
